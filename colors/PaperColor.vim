...
fun s:HL(group, fg, bg, attr)
  ...
  elseif s:use_256_color " 256-color Terminal
    ...
    if !empty(a:fg)
      let l:highlight .= " ctermfg=" . a:fg[-1] " use the last item in the array
    endif
    if !empty(a:bg)
...
fun! s:convert_from_256_to_hex(xterm) 
endfun
      ...
      if has_key(theme, l:variant)
        if has_key(theme[l:variant], 'TEST_256_COLOR_CONSISTENCY') && 
          ...
          for [l:color, l:value] in items(l:palette)
            ...
            let l:expected_hex = s:xterm_colors[l:value_256]
